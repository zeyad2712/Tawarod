...
$select_prj="SELECT * FROM `project` WHERE `client_id`=$client_id";
$run_prj=mysqli_query($connect , $select_prj);
...
if(isset($_POST['submit'])){
    ...
    if(!empty($project_id) &&!empty($post_desc) &&!empty($cat_id)){

        $insert="INSERT INTO `posts` VALUES (NULL, '$cat_id','$project_id','$post_desc')";
        $run_insert=mysqli_query($connect,$insert);
        header("location:posts.php?page_nr=1");   
    }else{
...
<div class="popup-content" >
    ...
    <form method="POST" name="category">
        ...
        <input id="postContent" name="post_desc" required value="<?php echo $post_desc ?>"></input>
...
<?php } ?>
        ...
        <div class="dropdownlist">
            ...
            <option disabled hidden selected>Project Name</option>
            <?php  foreach($run_prj as $data){?>

    <option name="project" value="<?php echo $data['project_id']; ?>" >
    ...
    <?php } ?>
        ...
        <button  name="submit" type="submit" class="add">Add Post</button>
    </form>
</div>
</div>
</body>
