...
$errors = array();
    ...
    if ($run_select && mysqli_num_rows($run_select) > 0) {
        ...
        $client_image = $array['client_image'];
    } else {
        ...
        $client_image=$_FILES['image']['name'];
    if(empty($client_image)){
        $update="UPDATE `client` SET `client_name`='$client_name' , `client_email`='$client_email'  WHERE `client_id`=$client_id";
         $run_update = mysqli_query($connect, $update);
         header('location:clientprofile.php');
    }else{
        $update="UPDATE `client` SET `client_name`='$client_name' , `client_email`='$client_email',
      `client_image`='$client_image' WHERE `client_id`=$client_id";
         $run_update = mysqli_query($connect, $update);
         move_uploaded_file($_FILES['image']['tmp_name'],"images/".$client_image);
         header('location:clientprofile.php');
    }
    }
...
<head>
    ...
    <!-- css -->
    <link rel="stylesheet" href="./css/edit_profile_client.css">
                        ...
                        <div class="account-settings">
                            <div class="user-profile">
                                <div class="user-avatar">
                                    <img src="<?php echo !empty($client_image) ? 'images/' . $client_image: 'https://bootdey.com/img/Content/avatar/avatar7.png'?> " alt="Maxwell Admin">
                                </div>
                                <form action="" method="post" enctype="multipart/form-data">
                                <label class="filelabel" for="photo"><i class="fa-sharp fa-solid fa-pen"></i> Edit your
                                    profile
                                    picture</label>
                                <br>
                                <input class="fileinput" type="file" name="image" id="photo"src= "<?php echo $client_image ?>">
                                
                            </div>
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
                                <div class="form-group">
                                    <label for="name">Full Name</label>
                                    <input type="text" class="form-control" id="name" name="name" value="<?php echo $client_name ?>">
                                </div>
                            </div>
                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
                                <div class="form-group">
                                    <label for="email">Your Email</label>
                                    <input type="email" class="form-control" id="email" name="email" value="<?php echo $client_email ?>">
                                </div>
                            </div>
                            <div class="row gutters">
                                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                    <div class="text-right">
                                         <button type="submit" id="cancel" name="cancel" class="btn btn-secondary"><a href="clientprofile.php">Cancel</a></button>
                                        <!-- ALBUTTON -->
                                    
                                        <button type="submit" id="submit" name="update" class="btn btn-primary">Save Changes</button> 
                                    </div>
                                    </form>
                                </div>
                            </div>
